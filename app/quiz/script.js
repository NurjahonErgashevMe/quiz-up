...
document.getElementById("start-quiz").addEventListener("click", async () => {
  ...
  const hasTakenTest = await checkIfStudentHasTakenTest(
    studentFullName,
    studentGrade,
    studentSubject
  );
...
function readQuizTableFromExcel(filePath) {
  ...
  return {
    questions: questionsData
      .filter((row) => row["savol"] && row["variantlar"] && row["tog'ri javob"])
      .map((row, index) => ({
        id: index + 1,
        question: row["savol"],
        options: row["variantlar"]
          ? row["variantlar"].split(",").map((opt) => opt.trim())
          : [],
        correctIndex: parseInt(row["tog'ri javob"], 10),
      })),
    totalTimeLimit: questionsData[0]["umumiy vaqt"]
      ? parseInt(questionsData[0]["umumiy vaqt"], 10) * 60
      : 3600,
  };
}
...
async function initializeQuiz() {
  try {
    ...
    alert.show(
      "Ошибка при загрузке вопросов. Проверьте формат файла Excel.\n\nДетали ошибки: " +
        error.message,
      "error"
    );
  }
...
function showQuestion() {
  ...
  const questionText = autoConvertToLatex(questionObj.question);

  // Используем KaTeX для форматирования вопроса, если он содержит математические выражения
  //   const questionText = questionObj.question.replace(
  //     /(cos|sin|tan|log|ln|sqrt|pi)(\d+)/g,
  //     "$1($2^\\circ)"
  //   );
  document.getElementById("question").innerHTML = katex.renderToString(
    questionText,
    {
      throwOnError: false,
    }
...
async function saveQuizResults(
  ...
  try {
    ...
    const performance =
      percentage >= 80 ? "great" : percentage >= 50 ? "good" : "bad";
    const now = new Date();
    const finishedAt = {
...
async function fetchStudentsForClass(grade) {
  try {
    ...
    alert.show(
      "Ошибка при загрузке данных учеников. Проверьте формат файла Excel.\n\nДетали ошибки: " +
        error.message,
      "error"
    );
    return [];
...
function extractMathSymbols(input) {
  // Регулярное выражение для поиска математических символов и выражений
  const mathPattern = /[+\-*/=()^√π!%]|\b(cos|sin|tan|log|ln|sqrt)\b|\d+/gi;

  // Ищем все совпадения
  const matches = input.match(mathPattern);

  // Если совпадения найдены, возвращаем их как строку, иначе пустую строку
  return matches ? matches.join(" ") : "";
}

// Пример использования:
const input = "Решите уравнение: 2x + 3 = 15, где x = √(9)";
const mathSymbols = extractMathSymbols(input);
console.log(mathSymbols); // "2 + 3 = 15 √ ( 9 )"

function autoConvertToLatex(input) {
  // 1. Регулярное выражение для тригонометрических функций (например, cos60, sin45)
  const mathPattern = /(\b(cos|sin|tan|log|sqrt)\b)(\d+)/gi;

  // 2. Замена тригонометрических функций на LaTeX-формат
  let latexOutput = input.replace(mathPattern, (match, func, angle) => {
    return `\\${func}(${angle}^\\circ)`;
  });

  // 3. Замена арифметических операторов (например, +, -, *, /)
  latexOutput = latexOutput.replace(/\+/g, " + ");
  latexOutput = latexOutput.replace(/-/g, " - ");
  latexOutput = latexOutput.replace(/\*/g, " \\cdot ");
  latexOutput = latexOutput.replace(/\//g, " \\div ");

  // 4. Оборачивание математической части в $ ... $
  latexOutput = latexOutput.replace(
    /([+\-*/]|\\(cos|sin|tan|log|sqrt)\(.*?\))/g,
    (match) => {
      return ` $${match}$ `;
    }
  );
  ...
  latexOutput = latexOutput.replace(/\s+/g, " ").trim();

  // 6. Если вопрос заканчивается знаком вопроса, добавляем его после формулы
  if (latexOutput.endsWith("?")) {
    latexOutput = latexOutput.slice(0, -1).trim() + " ?";
  }
...
async function checkIfStudentHasTakenTest(fullName, grade, subject) {
  try {
    ...
    return existingData.result.some(
      (result) => result.name === firstName && result.lastname === lastName
    );
  } catch (error) {
